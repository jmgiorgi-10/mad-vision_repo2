...
SUPABASE_URL = "https://vmhijchmwrekzdqkdmrf.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZtaGlqY2htd3Jla3pkcWtkbXJmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjcxMjMzNzYsImV4cCI6MjA0MjY5OTM3Nn0.XTOUz1gBrPVzQkv4EwQ3eZkgF6gu7KgvAPPRnmgFq4I"
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                result = await response.json()
                content = result['choices'][0]['message']['content'] # selects the content from the gpt-4 response
...
async def update_supabase(url, video_id, video_path, clips):
    ...
    #### EDIT SUPABASE PROJECT BASED ON THE BELOW ####
